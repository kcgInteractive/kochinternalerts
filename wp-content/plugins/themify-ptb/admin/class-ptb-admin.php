...
class PTB_Admin {
	...
	public function display_custom_post_types() {
		...
		do_action( 'themify_ptb_after_admin_page' );
	...
	public function display_templates() {
		...
		if ( isset( $_GET['action'] )) {
			...
			if ( 'delete' === $action ) {
				...
				wp_enqueue_style( 'themplate', plugin_dir_url( __FILE__ ) . 'css/ptb-themplate.css', array(), $this->version, 'all' );
				$this->ptb_wp_editor();
				wp_enqueue_script( 'themplate', plugin_dir_url( __FILE__ ) . 'js/ptb-themplate.js', array( 'jquery' ), $this->version, false );
	...
	public function display_custom_css() {
		...
		<div class="wrap">
			...
			<p>The Custom CSS added here will append to the site. It is mainly used by developers to add custom styling.</p>
...
<h3>Step-by-Step Overview</h3>
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( 'themify-icons', plugin_dir_url( __FILE__ ) . 'themify-icons/themify-icons.css', array(), $this->version, 'all' );
		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/ptb-admin.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/ptb-admin.js', array( 'jquery' ), $this->version, false );
	...
	public function add_ptb_shortcode() {
		...
		if ( 'true' == get_user_option( 'rich_editing' ) ) {
			add_filter( 'mce_external_plugins', array( $this, $this->plugin_name . '_add_shortcodes_buttons' ) );
			add_filter( 'mce_buttons', array( $this, $this->plugin_name . '_register_button' ) );
			add_action( 'admin_footer', array( $this, $this->plugin_name . '_get_shortcodes' ) );
		}
	}
	...
	public function ptb_get_shortcodes() {
		...
		echo '<script type="text/javascript">
                        ...
                        var $_url = "' . admin_url( 'admin-ajax.php?action=' . $this->plugin_name . '_ajax_get_post_type' ) . '";';
		...
		foreach ( $themplates as $k => $post_themes ) {
			if ( $post_themes->has_archive() ) {
				...
				echo "shortcodes_button['$k'] ={'type':'{$post_type}','name':'{$custom_post->plural_label[$lang]}'};";
			}
		}
		echo '</script>';
	}
