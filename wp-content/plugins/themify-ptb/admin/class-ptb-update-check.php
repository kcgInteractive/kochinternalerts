...
class PTB_Update_Check {
	...
	public function __construct( $name, $version, $slug ) {
		...
		if( isset( $_GET['page'] ) && ! isset( $_GET['action'] ) && in_array( $_GET['page'], array( 'ptb-cpt', 'ptb-ctx', 'ptb-ptt', 'ptb-ie', 'ptb-css' ) ) ) {
			add_action( 'admin_notices', array( $this, 'check_version' ), 3 );
			add_action( 'admin_enqueue_scripts', array( $this, 'enqueue' ) );
		} elseif( isset( $_GET['page'] ) && isset( $_GET['action'] ) && in_array( $_GET['page'], array( 'ptb-cpt', 'ptb-ctx', 'ptb-ptt', 'ptb-ie', 'ptb-css' ) ) ) {
		...
		add_action( 'themify_ptb_after_admin_page', array( $this, 'prompt' ) );
	}
	...
	function themify_ptb_updater(){
		...
		$upgrader = new PTB_Upgrader( new Plugin_Upgrader_Skin(
			array(
				'plugin' => $plugin_slug,
				'title' => __( 'Update Post Type Builder', 'ptb' )
			)
		));
...
function themify_ptb_upgrade_complete($update_actions, $plugin) {
	if ( $plugin == 'post-type-builder/post-type-builder.php' ) {
