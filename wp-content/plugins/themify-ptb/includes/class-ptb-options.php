...
class PTB_Options {
	...
	public function ptb_register_custom_post_types() {
		...
		foreach ( $cpt_objects_array as $cpt_object ) {
			...
			add_action( 'add_meta_boxes_' . $cpt_object->slug, array( $this, 'add_custom_meta_boxes' ), 10, 1 );
                ...
                add_shortcode( 'ptb', array( $this, 'ptb_shortcode' ) );
	...
	public function ptb_ajax_theme() {
		...
		if ( isset( $_REQUEST['template'] ) && $_REQUEST['template'] ) {
			...
			if ( class_exists( $class ) && $_REQUEST[ $this->plugin_name . '-' . $class::$key ] ) {

				$themplate_id = $_REQUEST[ $this->plugin_name . '-' . $class::$key ];
	...
	public function ptb_ajax_get_post_type() {
		if ( isset( $_POST['post_type'] ) && $_POST['post_type'] ) {
			...
			$fireds   = $grids = $by = array();
			foreach ( $sortable as $s ) {
				$fireds[] = array( 'text' => ucfirst($s), 'value' => $s );
			}
			...
			foreach ( $archive as $key => $arh ) {
				...
				if ( $key == 'order' ) {
					...
					$result['data'][ $key ]['type'] = 'textbox';
                        ...
                        $result['title'] = __('PTB Shortcode',  'ptb');
	...
	public function edit_custom_post_type( $id, $cpt, $continue = true ) {
		...
		if ( $this->has_custom_post_type( $id ) ) {
			...
			$continue && $this->synchronize_post_type_to_taxonomy( $id, $cpt );
	...
	public static function get_cmb_types() {
		...
		$cmb_types = array(/*
			'ptb_cmb_select'       => array(
				'name' => 'Select'
			),
			'ptb_cmb_checkbox'     => array(
				'name' => 'Checkbox'
			),
			'ptb_cmb_radio_button' => array(
				'name' => 'Radio Button'
			),
			'ptb_cmb_image'        => array(
				'name' => 'Image'
			),
			'ptb_cmb_link_button'  => array(
				'name' => 'Link Button'
			)
*/
		);

		return apply_filters( 'ptb_cmb_types', $cmb_types );
	...
	public function add_custom_meta_box_cb( $post ) {
		...
		<div class="ptb_post_cmb_wrapper">
			...
			foreach ( $cmb_options as $meta_box_id => $args ) {
				...
				?>
				<div class="ptb_post_cmb_item_wrapper" data-ptb-cmb-type="<?php print( $args['type'] ) ?>"
	...
	public function update_post_type_template( $ptt ) {
		...
		$this->option_post_type_templates[ $ptt_id ] = $ptt->serialize();
	...
	public function ptb_shortcode( $atts ) {
		...
		if ( null == $template ) {
                    //_e( 'There is no template for "' . $type . '" post type. Please update type attribute of shortcode.', 'ptb');
                    return;
		} 
		// WP_Query arguments
		$args  = array(
		...
		if ( $query->have_posts() ) {
			...
			$themplate_layout = $template->get_archive();
			...
			while ( $query->have_posts() ) {
				...
				$post_meta['taxonomies'] = wp_get_post_terms( get_the_ID(), $post_taxonomies );
				$post_meta               = array_merge( $post_meta, get_post_custom(), get_post( '', ARRAY_A ) );
                                
				$html .= '<article itemscope itemtype="http://schema.org/Article" class="'.$this->plugin_name.'_post clearfix">';
	...
	public function ptb_filter_post_type_content( $content ) {
		$post_type = get_post_type();

		$templateObject = $this->get_post_type_template_by_type( $post_type );

		if ( is_admin() || is_null( $templateObject ) ) {
	...
	public function ptb_filter_post_type_content_post( $content ) {
		...
		if ( $single || $archive ) {
			...
			if ( isset( $themplate_layout['layout'] ) ) {
				return $themplate->display_public_themplate( $themplate_layout['layout'], $post_support, $cmb_options, $post_meta, $post_type, true );
			}
		}
	...
	public function ptb_filter_post_type_exceprt_post( $content ) {
		...
		elseif(self::$render_instance=='content'){
			return '';
		}
		
		self::$render_instance = 'exceprt';
		$single  = $templateObject->has_single() && is_singular( $post_type );
