...
class PTB_Utils {
	...
	public static function get_all_languages() {
		...
		if ( defined( 'ICL_LANGUAGE_CODE' ) ) {
			...
			$all_lang = icl_get_languages( 'skip_missing=N&orderby=KEY&order=DIR&link_empty_to=str' );
			...
			$all_lang                           = substr( get_bloginfo( 'language' ), 0, 2 );
	...
	public static function get_label_ml( $label, &$labels_ml ) {
		...
		$labels_ml[ self::get_current_language_code() ] = $label;

		return $labels_ml;

	}

	public static function get_label( &$labels_ml ) {

		$clc = self::get_current_language_code();

		if ( ! is_array( $labels_ml ) ) {

			$empty_array = array();

			$labels_ml = self::get_label_ml( $labels_ml, $empty_array );

		}

		if ( ! array_key_exists( $clc, $labels_ml ) ) {

			//$labels_ml[$clc] = __( '[Not translated]' );
			$default_lng       = self::get_default_language_code();
			$labels_ml[ $clc ] = isset( $labels_ml[ $default_lng ] ) ? $labels_ml[ $default_lng ] : '';

		}

		return $labels_ml[ $clc ];
