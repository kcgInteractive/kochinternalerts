...
class PTB_List_Table_CPT extends WP_List_Table {
	...
	private function prepare_data() {
		...
		foreach ( $custom_post_types as $cpt ) {
			...
			$cpt_data                   = array();
			$cpt_data['ID']             = $cpt->id;
			$cpt_data['singular_label'] = isset($cpt->singular_label[ $lng ])?$cpt->singular_label[ $lng ]:'';
	...
	function column_singular_label( $item ) {
		...
		$actions = array(
			'edit'   => sprintf(
				'<a href="?page=%1$s&action=%2$s&post_type=%3$s" data-slug="%3$s" class="ptb_cpt_edit">Edit</a>',
				$_REQUEST['page'],
				'edit',
				$item['ID']
			),
			'delete' => sprintf(
				'<a href="?page=%1$s&action=%2$s&post_type=%3$s" data-slug="%4$s" class="ptb_cpt_delete">Delete</a>',
				$_REQUEST['page'],
				'delete',
				$item['singular_label'],
				$item['ID']
